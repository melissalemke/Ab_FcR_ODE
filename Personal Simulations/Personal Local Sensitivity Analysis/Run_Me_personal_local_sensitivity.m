...
% This code uses the paralell computing toolbox to run simulations in
% parallel
...
function Run_Me_personal_local_sensitivity()
clear();
delete(gcp('nocreate'))
tic()
...
for fcr_idx = 1:length(FcR_kon(1,:)) % columns in the spreadsheet
    ...
    for p_num = 1:length(patient_id)
        ...
        dParams= [0.004 0.02 0.1 2.5 5 20];
        [ysense, sensfilettl] = Sensitivity_1D(params, dParams, paramnames,length(ybase(1,:)));
        ...
        all_ysense(fcr_id,p_num,:,:) = ysense;
    end
end
...
save([strain,'_personal_local_sensitivity_all_fcrs_IgG_v1-v105_',datestr(today()),'.mat'],...
    'all_run', 'param_idv','complexname','paramnames', 'IgG_FcR_data',...
    'dParams', 'all_ysense','patient_id','FcR_names');
toc()
end
